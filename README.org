...
It has type-safe arithmetic, SIMD, fast syscalls, interfaces (also known as facades),
...
* Building
libCat requires a development version of GCC 15 from the =trunk= branch. Compiling libCat is only tested using =ninja=.
