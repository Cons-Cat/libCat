...
** =cat-reflow-comments=
#+BEGIN_SRC sh
  cmake --build build/ --target cat-reflow-comments
#+END_SRC
=clang-format= does not aggressively merge erroneous line breaks in comments, which are
common after mass refactoring. =cat-reflow-comments= performs more aggressive formatting
of comments alone, but like clang-format it respects blank lines between paragraphs and
URL coherence.
...
#+BEGIN_SRC sh
  cmake --build build/ --target cat-reflow-comments-check
#+END_SRC
The read-only variant, same idea as =cat-format-check=, reports =would reflow: ...=
for any file where // reflow would change. Run =cat-reflow-comments=, then
=cat-format=, to fix drift.
...
** Tooling overrides
- =CAT_LIBCAT_JOBS= - Worker count for =cat-format=, =cat-reflow-comments=, and
  =cat-reflow-comments-check= (capped at 4 in the scripts to limit RAM use, e.g. on
...
- =CAT_CLANG_FORMAT_PATH= - Path to a specific =clang-format= binary
  ...
  the =cat-format= / =cat-format-check= targets. =cat-reflow-*= is separate, uses
