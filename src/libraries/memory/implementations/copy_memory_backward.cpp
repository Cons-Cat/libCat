...
void
copy_memory_forward_overlap_impl(byte const* _Nonnull p_source,
...
void
copy_memory_backward_impl(byte const* _Nonnull p_source,
                          byte* _Nonnull p_destination, idx bytes) {
   ...
   $simd_switch($abi(avx2,
                     {
                        copy_memory_backward_large<char1x_>(
                           p_source, p_destination, bytes);
                        x64::zero_upper_avx_registers();
                     }),
                $abi(sse2, {
                   copy_memory_backward_large<char1x_>(p_source, p_destination,
                                                       bytes);
                }));
}
