...
[[nodiscard]]
inline auto
compare_memory_chunk(byte const* _Nonnull p_left, byte const* _Nonnull p_right,
...
inline auto
compare_memory_small(byte const* _Nonnull p_lhs, byte const* _Nonnull p_rhs,
                     idx bytes) -> std::strong_ordering {
   ...
   auto compare_head_tail = [&]<idx chunk_bytes> {
      ...
      __builtin_memcpy_inline(&left_tail, p_left + byte_count - chunk_bytes,
                              chunk_bytes);
      __builtin_memcpy_inline(&right_head, p_right, chunk_bytes);
      __builtin_memcpy_inline(&right_tail, p_right + byte_count - chunk_bytes,
                              chunk_bytes);
      ...
      if (left_tail != right_tail) {
         return compare_memory_chunk(p_left + byte_count - chunk_bytes,
                                     p_right + byte_count - chunk_bytes,
                                     chunk_bytes);
      }
   ...
   if (byte_count == 16u) {
      if (auto result = compare_memory_word(p_left, p_right);
   ...
   if (auto result = compare_memory_word(p_left + 8u, p_right + 8u);
       result != std::strong_ordering::equal) {
      return result;
   }

   if (auto result = compare_memory_word(p_left + byte_count - 16,
                                         p_right + byte_count - 16);
       result != std::strong_ordering::equal) {
      return result;
   }

   return compare_memory_word(p_left + byte_count - 8,
                              p_right + byte_count - 8);
}
