...
struct take_last_view_impl : iterable_interface<> {
   Base m_base;
   idx m_count;
 ...
 public:
   ...
   iterate() {
      ...
      if constexpr (is_collection<decltype(collection)>) {
         ...
         idx const dropped = count > m_count ? idx(count - m_count) : 0u;
...
template <typename Base>
using take_last_view = take_last_view_impl<Base>;

struct take_last_impl : view_interface<take_last_impl> {
   idx count;
   ...
   constexpr auto
   apply(Iterable&& incoming) const -> take_last_view<Iterable> {
      return take_last_view<Iterable>{{}, $fwd(incoming), count};
   }
};
}  // namespace detail

// Lazily yield the last `count` elements. This view adaptor requires a
// collection or a sized iterable so it can locate the suffix without buffering.
[[gnu::always_inline, gnu::nodebug]]
constexpr auto
take_last(idx count) -> detail::take_last_impl {
   return detail::take_last_impl{{}, count};
}
