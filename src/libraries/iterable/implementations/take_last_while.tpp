...
struct take_last_while_view_impl : iterable_interface<> {
   Base m_base;
   Callback m_callback;
...
template <typename Base, typename Callback>
using take_last_while_view = take_last_while_view_impl<Base, Callback>;

template <typename Callback>
struct take_last_while_impl : view_interface<take_last_while_impl<Callback>> {
   Callback callback;
   ...
   apply(
      Iterable&& incoming
   ) const& -> take_last_while_view<Iterable, Callback> {
      return take_last_while_view<Iterable, Callback>{
         {},
         $fwd(incoming),
         callback,
      };
   ...
   constexpr auto
   apply(Iterable&& incoming) && -> take_last_while_view<Iterable, Callback> {
      return take_last_while_view<Iterable, Callback>{
         {},
         $fwd(incoming),
         move(callback),
...
// Lazily yield the trailing elements accepted by `callback`. This view adaptor
// requires a collection so it can locate the suffix without buffering elements.
template <typename Callback>
[[gnu::always_inline, gnu::nodebug]]
constexpr auto
take_last_while(Callback callback) -> detail::take_last_while_impl<Callback> {
   return detail::take_last_while_impl<Callback>{{}, move(callback)};
}
