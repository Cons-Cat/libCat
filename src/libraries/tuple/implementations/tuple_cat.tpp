...
forward_tuple(Tuple&& input_tuple) {
   ...
   return forward_tuple_impl(
      $fwd(input_tuple), make_index_sequence<tuple_decay::types::size()>{}
   );
}

template <
   typename LeftTuple, typename RightTuple, idx... left_index,
   idx... right_index>
constexpr auto
tuple_cat_two_impl(
   LeftTuple&& left, RightTuple&& right, index_list_type<left_index...>,
   ...
   return tuple<
      typename left_types::template get<left_index>...,
...
tuple_cat_two(LeftTuple&& left, RightTuple&& right) {
   ...
   return tuple_cat_two_impl(
      $fwd(left), $fwd(right), make_index_sequence<left_decay::types::size()>{},
...
template <typename LeftTuple, typename RightTuple, typename... Remaining>
   requires(
      is_tuple<LeftTuple> && is_tuple<RightTuple>
      && (is_tuple<Remaining> && ...)
   )
constexpr auto
