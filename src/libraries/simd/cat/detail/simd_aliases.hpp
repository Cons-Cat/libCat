...
//
//   * `native_simd<T>` / `native_simd_mask<T>` pin to `ABI<T>`.
//
//   * `deduce_simd<T, lanes>` / `deduce_simd_mask<T, lanes>` pass `ABI<T>` to
//     `simd_abi::deduce` as an extra candidate, biasing the deducer toward the
//     arch.
//
//   * `fixed_size_simd<T, lanes>` and `scalar_simd<T>` are independent
//     of `ABI` (they always use `simd_abi::fixed_size` / `simd_abi::scalar`).
//
//   * Per-element aliases (`int4x_`, `int1x16`, `float4x_`, ...) are spelled in
//     terms of those four families, so they pick up the `ABI` substitution too.
#define CAT_SIMD_ALIASES(ABI)                                                 \
   ...
   /* matching arch wins. The `_x_` placeholder-lane aliases live in the  */  \
   /* separate `CAT_SIMD_PLACEHOLDER_ALIASES` macro. Those vary directly  */  \
   /* with `ABI<T>` and only make sense inside an arch namespace, where   */  \
   /* `ABI` is per-arch.                                                  */  \
                                                                              \
   /* int1 / uint1 vectors: */                                                \
