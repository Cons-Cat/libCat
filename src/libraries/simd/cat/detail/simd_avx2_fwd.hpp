...
requires(cat::is_same<typename abi_type::scalar_type, T>) class alignas(
...
template <typename T>
[[nodiscard]] auto testz(cat::simd_mask<x64::avx2_abi<T>, T> left,
...
namespace cat {
template <typename T>
[[nodiscard]] auto all_of(simd_mask<x64::avx2_abi<T>, T> mask) -> bool;

template <typename T>
[[nodiscard]] auto any_of(simd_mask<x64::avx2_abi<T>, T> mask) -> bool;

template <ssize bits_count>
requires(bits_count > 0) class bitset;
