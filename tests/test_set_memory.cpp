...
auto main() -> int {
    ...
    verify(p_page[1000] == 1_u1);

    // Test that unaligned memory still sets correctly.
    cat::set_memory(p_page + 3, 2_u1, 2_ki - 6);
    verify(p_page[0] == 1_u1);
    verify(p_page[2] == 1_u1);
    verify(p_page[3] == 2_u1);
    verify(p_page[(2_ki - 4).raw] == 2_u1);
    verify(p_page[(2_ki - 3).raw] == 1_u1);

    // Test zeroing out memory.
    cat::zero_memory(p_page, 4_ki);
    verify(p_page[0] == 0_u1);
    verify(p_page[(4_ki).raw - 1] == 0_u1);

    // Test setting values larger than 1 byte.
    cat::set_memory(p_page, 1_i2, 2_ki);
    verify(static_cast<int2*>(static_cast<void*>(p_page))[10] == 1_i2);
    // The next byte after this should be 0.
    verify(static_cast<int1*>(static_cast<void*>(p_page))[21] == 0);

    cat::set_memory(p_page, 1_i4, 1_ki);
    verify(static_cast<int4*>(static_cast<void*>(p_page))[10] == 1_i4);
