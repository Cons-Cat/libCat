...
$test(maybe_reference_dangling) {
   ...
   // `int const&` binding to a pr-value would create a temporary (rejected).
   static_assert(!cat::is_constructible<cat::maybe<int const&>, int>);
...
$test(maybe_nested_construction) {
   ...
   // From an engaged `maybe<int>` l-value: the outer must engage and hold the
   ...
   // From a disengaged `maybe<int>` l-value: the outer is still engaged, and
...
$test(maybe_errdefer) {
   ...
   run([&]() -> cat::maybe<int4> {
      ...
      cat::maybe<int4> empty = cat::nullopt;
      return $prop(empty);
   });
   run([&]() -> cat::maybe<int4> {
      $errdefer {
      ...
      cat::maybe<int4> empty = cat::nullopt;
      return $prop_as(empty, cat::nullopt);
   });
   run([&]() -> cat::maybe<int4> {
      $errdefer {
         ++prop_or_fail;
      ...
      cat::maybe<int4> empty = cat::nullopt;
      return $prop_or(empty, 99_i4);
   });

   cat::verify(prop_success == 0);
   cat::verify(prop_fail == 1);
   cat::verify(prop_as_fail == 1);
   cat::verify(prop_or_fail == 1);
}

// Each guard snapshots the counter at its own construction site, so guards
// installed after a successful `$prop` do not retroactively fire, all
// guards alive at failure time fire exactly once, and they unwind in
// reverse construction order.
$test(maybe_multiple_errdefers) {
   int4 order = 0;
   int4 early = 0;
   int4 middle = 0;
   int4 late = 0;

   auto wrapper = [&]() -> cat::maybe<int4> {
      $errdefer {
         early = ++order;
      };
      cat::maybe<int4> engaged = 1_i4;
      int4 first = $prop(engaged);
      $errdefer {
         middle = ++order;
      };
      $errdefer {
      ...
      cat::maybe<int4> empty = cat::nullopt;
      return first + $prop(empty);
   };

   cat::verify(!wrapper().has_value());
   cat::verify(late == 1);
   cat::verify(middle == 2);
   cat::verify(early == 3);
}
