...
test(bitset) {
   ...
   // bits).
   constexpr cat::bitset<8u> bits8_rot =
      cat::make_bitset<8u>(0b0000'0001_u1);
   ...
   // Test `.set_even()`, `.set_odd()`, `.unset_even()`, `.unset_odd()`, and
   // the `make_bitset_even()` / `make_bitset_odd()` factories.
   ...
   // Test bitwise operators generated by `bitwise_interface`, and the
   // explicit `operator^` / `operator^=` overloads.
   ...
   // `^` of two equal-valued bitsets is all zero; `^` with a zero bitset is
   // the identity.
