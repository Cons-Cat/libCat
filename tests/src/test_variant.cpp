...
$test(variant_constexpr) {
   ...
   constexpr int visited = cat::visit(
      [](auto value) -> int {
         using value_type = __typeof_unqual(value);
...
$test(variant_visit_multi) {
   ...
   cat::visit(
      [&](auto& a_value, auto& b_value) {
         using A = __typeof_unqual(a_value);
         using B = __typeof_unqual(b_value);
   ...
   int sum = cat::visit(
      [](auto& x, auto& y, auto& z) -> int {
         int total = 0;
         if constexpr (cat::is_same<__typeof_unqual(x), int>) {
            total += x;
         }
         if constexpr (cat::is_same<__typeof_unqual(y), char>) {
            total += y;
         }
         if constexpr (cat::is_same<__typeof_unqual(z), int>) {
            total += z;
         }
         return total;
      },
...
$test(variant_inheritance_shadowing) {
   ...
   int seen = sp.visit([](auto& value) -> int {
      if constexpr (cat::is_same<__typeof_unqual(value), int>) {
         return value;
      }
      return -2;
   });
