...
#
# `cmake --install build [--prefix <sysroot>]` lays out the libCat tree so an
# external CMake project can do.
#
...
# `-include global_includes.hpp`, `_start.cpp`, `libcat.ld`) the in-tree tests /
# examples get from `cat`'s `INTERFACE`. The export set is named `catTargets`.
# `catConfig.cmake`/`catConfigVersion.cmake` are generated from
# `cmake/catConfig.cmake.in`.
#
...
#
# Registering these rules unconditionally has no cost on a regular build --
# CMake only executes them on `cmake --install`/`ninja install`. Consumers who
# never install pay nothing.
...
# dirs in `src/CMakeLists.txt` point at exactly these paths, so the `.cpp`
...
# `INSTALL_INTERFACE` link options as
# `${CMAKE_INSTALL_FULL_DATADIR}/libcat/libcat.ld`, so install it there.
