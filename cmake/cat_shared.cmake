...
# `cat-impl-shared`, or via the `CAT_USE_SHARED` switch keep linking `cat` and
# have it transparently resolve to the `.so`.
#
...
#     without it).
#
# Usage requirements (essentials, include dirs, link options, `_start.cpp`,
# linker-script `LINK_DEPENDS`) propagate via INTERFACE, so consumers get a
...
# `cat-impl-shared` does too. `PRIVATE` for the build-only specialisations
...
target_compile_options(cat-impl-shared
  ...
  PRIVATE
    ...
    # `-flto=auto`) to keep `libcat.a`'s symbol set tight, which strips libCat's
    # entire ABI from the `.so` if not undone here. Trailing
...
# `src/libraries/{string,math}/implementations/`, but consumers can still end up
# referencing other libC / libM symbols (e.g. compiler-emitted `printf`, `sin`,
# `cos`, `log`). Explicit `-lm -lc` makes those resolvable both at the `.so`'s
# own link and at the consumer's. PUBLIC is required because `lld` does not
# follow a shared library's `DT_NEEDED` entries when resolving the consumer's
# undefined symbols, so each consumer needs `-lm -lc` repeated on its own link
# line.
...
# (`__asan_init`, `__asan_register_elf_globals`, ...). `clang-repl` itself is
# not asanified, so those references stay unresolved unless the ASan runtime is
# `LD_PRELOAD`' d into the REPL process. The wrapper handles this automatically
# when `CAT_ASAN_RUNTIME_PATH` is non-empty. The path.
...
# `USES_TERMINAL` puts the command in Ninja's `console` pool so stdin / stdout
# pass through unbuffered -- required for an interactive REPL. The explicit
# `add_dependencies` ensures `libcat.so` exists before the wrapper tries to
