...
# recompiles `cat-impl`'s sources with `-fsave-optimization-record`, so
# Clang writes a `*.opt.yaml` next to every object file.
# `EXCLUDE_FROM_ALL` keeps it off the default build.
#
# An OBJECT library (rather than a custom target driving the compiler
# by hand) keeps CMake in charge of compile rules -- include dirs,
# definitions, `REUSE_FROM` PCH, the sccache launcher -- so the flag
# drops in with one line. It PRIVATE-links `cat` for the essentials
# and re-applies `CAT_COMPILE_OPTIONS_INTERNAL` because those are
# PRIVATE on `cat-impl` and do not propagate.
#
# The companion `opt-viewer.py` invocation is located at configure time
# and echoed as a STATUS message so the user has a ready command to
# render the YAML records into an HTML report. Post-build `TARGET`
# custom commands on OBJECT libraries require CMake 3.29; the floor
# here is 3.24, so configure-time notice it is.
#
# Reference:
...
# Prefer any `opt-viewer.py` already on PATH so an upgraded script can
# be substituted without touching CMake. Otherwise fall back to the
# script shipped under `/usr/lib/llvm-<major>/share/opt-viewer/` that
# matches the configured compiler version. Debian / Ubuntu's
# `llvm-<N>-tools` installs the script without the executable bit, so
# prefix `python3` when the resolved path ends in `.py`.
