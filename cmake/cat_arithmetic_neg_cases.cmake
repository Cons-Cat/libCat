...
# are ill-formed are generated in `cat_arithmetic_neg_matrix.cmake` from the
# `deconst` / narrow-LHS / mixed-signed grid.

# Width-by-width OOR brace `uintN`/`intN` constants are emitted in
# `cat_arithmetic_negative.cmake` (needs `${n}` interpolation; CMake `[[`...
# `]]` is literal). Here: additional braced inits with out-of-range constants.
# Keep those as **true** constant expressions: OOR is validated via `consteval`
...
# *sound*).
_cat_neg_expect_illformed("enable-if-uint1-implicit-int-300" [[#include <cat/arithmetic>
void t() { cat::uint1 a = 300; }
]])
_cat_neg_expect_illformed("enable-if-int1-implicit-uint-300" [[#include <cat/arithmetic>
...
_cat_neg_expect_illformed("enable-if-idx-implicit-int-neg1" [[#include <cat/arithmetic>
void t() { cat::idx i = -1; }
]])
...
_cat_neg_expect_illformed("enable-if-uintptr-implicit-int-neg1" [[#include <cat/arithmetic>
...
_cat_neg_expect_illformed("undef-uint1-pluseq-int-neg" [[#include <cat/arithmetic>
...
# form in all builds that mirrors the trait; a dedicated `requires` suite can
# cover the type-trait, not a single -fsyntax-only TU.
...
# friend and `minus_interface` path through `subtract_by` (also integral) apply.
